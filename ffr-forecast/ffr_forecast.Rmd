...
forecast_set <- ffr_set_first_day %>%
  filter(between(date, date("2021-03-01"), max_date_ffr)) %>%
  rename(tr_mid_h0 = tr_mid) %>%
...
ffr_set_dummy_lag2 <- ffr_set_first_day %>%
  mutate(d_tr_mid_lag2 = dummy_of_variance_0(tr_mid, 2)) %>%
  filter(between(date, min_date_h0, max_date_h0)) %>%
  select(-tr_mid)
...
# Include the dummy variable in the dataset


```{r}

h0_set <- h0_set %>%
  inner_join(ffr_set_dummy_lag2, "date")
...
name_X_selected <- c("log_tr_mid_h0", "log_tr_mid_lag1", "log_tr_mid_lag2", "log_cpi_lag16", "cpi_lag12", "log_tr_mid_lag6", "log_cpi_lag12", "cpi_lag6", "log_cpi_lag6", "log_tr_mid_ma6", "log_tr_mid_ma3", "log_tr_mid_ma2", "d_tr_mid_lag2")
...
mod <- lm(tr_mid_h6 ~ log_tr_mid_ma3 + d_tr_mid_lag2, data = train_set)
